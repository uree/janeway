...
def create_paths():
    base_path = os.path.join(settings.BASE_DIR, 'static', 'javnozdravje')
    folders = ['css']
    ...
    for journal in journals:
        for file in journal.scss_files:
            if file.endswith('clean_override.css'):
                ...
                override_css_dir = os.path.join(settings.BASE_DIR, 'static', 'javnozdravje', 'css')
...
def build():
    ...
    create_paths()
    print('Copying CSS')
    copy_file('themes/javnozdravje/assets/css/javnozdravje.css', 'static/clean/css/javnozdravje.css')
    process_journals()
